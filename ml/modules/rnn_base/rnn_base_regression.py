    ...
    def _train_and_evaluate(self, fold=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, (inputs, labels) in enumerate(train_loader):
                ...
                self.evals_result['iteration_loss'].append(raw_loss.item())
            ...
            r2_train = 1 - torch.sum((all_train_labels - all_train_preds) ** 2) / torch.sum((all_train_labels - torch.mean(all_train_labels)) ** 2)
            self.evals_result['train_acc'].append(r2_train.item())
            ...
            if val_loader is not None:
                ...
                r2_val = 1 - torch.sum((all_val_labels - all_val_preds) ** 2) / torch.sum((all_val_labels - torch.mean(all_val_labels)) ** 2)
                self.evals_result['val_acc'].append(r2_val.item())
            else:
                val_epoch_loss = None
                val_epoch_rmse = None
                r2_val = None
            ...
            # -----------------------------
            if fold is None:
                checkpoint_path = model_save_dir / f"rnn_model_{self.target_column}_final_epoch{epoch+1}.pt"
                torch.save({
                    'epoch': epoch + 1,
                    ...
                    'train_acc': r2_train.item(),
                    'val_acc': r2_val.item() if r2_val is not None else None,
                    'hyperparameters': {
        ...
        if test_loader is not None:
            ...
            r2_test = 1 - torch.sum((all_test_labels - all_test_preds) ** 2) / torch.sum((all_test_labels - torch.mean(all_test_labels)) ** 2)
            test_acc = r2_test.item()

            self.evals_result['test_labels'] = all_test_labels.tolist()
            self.evals_result['test_preds'] = all_test_preds.tolist()
            self.evals_result['test_probas'] = all_test_preds.tolist()
            self.evals_result['test_rmse'].append(test_rmse)

            print(f"Test Loss: {test_loss:.4f}, Test R2: {test_acc:.4f}, Test RMSE: {test_rmse:.4f}")
        ...
        # -----------------------------
        if fold is None:
            model_save_path = model_save_dir / f"rnn_model_{self.target_column}_final.pt"
        ...
        return {"val_loss": best_val_loss, "test_loss": test_loss, "test_acc": test_acc, "test_rmse": test_rmse}
...
def aggregate_results(results: list):
    ...
    for res in results:
        ...
        print(f"フォールド {fold}: テスト損失 (multi_logloss) = {test_loss:.4f}, R2スコア (accuracy) = {test_acc:.4f}, RMSE = {test_rmse:.4f}")
    ...
    print(f"全体平均R2スコア: {avg_acc:.4f}")
    print(f"全体平均RMSE: {avg_rmse:.4f}\n")
...
def train_model_on_full_data(config: dict, final_hyperparams: dict) -> dict:
    ...
    results_save_path = results_save_dir / f"rnn_test_results_{config['target_column']}.pkl"
