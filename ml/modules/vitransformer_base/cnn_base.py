...
from modules.cnn_base.cnn_base_arc import VisionTransformer
...
class CNNBase(nn.Module):
    ...
    def _train_and_evaluate(self, fold=None):
        ...
        # 例: DataLoader (train/val/test の切り分けは実際のデータに合わせて設定)
        ...
        # === 改良版 VisionTransformer を使用 ===
        model = VisionTransformer(
            img_size=405,       # 入力画像のサイズ
            patch_size=15,      # パッチサイズ
            in_channels=1,      # 入力チャネル数（RGBなら3）
            num_classes=2,     # 分類先のクラス数
            embed_dim=768,      # 埋め込み次元
            depth=12,           # Transformer Encoder Layer の深さ
            num_heads=12,       # Multi-Head Attention のヘッド数
            mlp_ratio=4.0,      # MLP の拡張率 (embed_dim * mlp_ratio)
            drop_attn=0.1,      # Self-Attention のドロップアウト率
            drop_mlp=0.1,       # MLP 部分のドロップアウト率
            drop_path_rate=0.1, # Stochastic Depth のドロップ率（深さに応じて線形増加）
            use_2d_pos=True     # 2D Positional Embedding を使用するか
        )

        # Loss & Optimizer
        criterion = nn.CrossEntropyLoss()
        # AdamW が一般的: weight_decay を適度に (例: 1e-5)
        ...
        if test_loader is not None:
            ...
            all_labels = []
            all_preds = []
            all_probas = []  # ROC-AUC用に確率（softmaxなど）
